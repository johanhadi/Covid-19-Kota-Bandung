---
title: "Dashboard Covid-19 Kota Bandung (Update 16 Mei 2020)"
output: 
...
Rangkuman {data-icon="fas fa-home"}
=======================================================================
Row
-----------------------------------------------------------------------
...
leaflet(data = bubble) %>%
  ...
  addPolygons(data = batas, fillColor = "blue", weight = 2 , color = "black", dashArray = 3,
              ...
              labelOptions = labelOptions(
                ...
                textsize = "15px")) %>%
  addMinicharts(
    bubble$Long, bubble$Lat,
    chartdata = bubble$Total,
    time = bubble$Tanggal,
    width = 60*sqrt(bubble$Total)/sqrt(max(bubble$Total)),
    showLabels = TRUE, fillColor = confirmed_color
  )%>%
  addMarkers(
      lng = rujukan$Long, lat = rujukan$Lat, icon = ikonrs,
      popup = label4, group = "Rumah Sakit Rujukan",
      labelOptions = labelOptions(noHide = T, direction = "right",
                                  ...
                                  textsize = "15px")) %>%
  addLayersControl(
   overlayGroups = c("Rumah Sakit Rujukan"),options = layersControlOptions(collapsed = FALSE)) %>%
   addMiniMap(
    tiles = providers$CartoDB.Positron,
    toggleDisplay = TRUE, position = "bottomleft") 
```
...
plot_ly(data = daily) %>%
  ...
  layout(
    title = "",
    yaxis = list(title = "Kasus Sembuh", range = c(0,70)), 
    xaxis = list(title = "Tanggal"),
    legend = list(x = 0.1, y = 0.9),
    hovermode = "compare")
```
